...
class BookingSystem:
    ...
    def find_client(self, name, number):
        '''
        Find client by name and contact number
        ...
        for appt in self.appointments_list:
            if appt.client.get_name() == name:
                ...
                if new_type == appt.appointment_type.lower():
                    if new_type == "gp":
                        appt.reason_for_visit = input(f"Enter a new reason (press enter to keep it as '{appt.reason_for_visit}'): ") or appt.reason_for_visit
                        while True:
                            appt.consultation_type = input(f"Enter new consultation type (Telehealth/Standard/Long) or press Enter to keep ({appt.consultation_type}): ").strip().lower() or appt.consultation_type
                            if appt.consultation_type in ["telehealth", "standard", "long"]:
                                break
